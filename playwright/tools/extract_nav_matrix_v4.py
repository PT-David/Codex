...
from extract_toc_universal_v7 import (
    _safe_name_from_url,
    _render_md,
...
def _normalize_part_toc(part_title: str, part_url: str, toc_tree: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    把 "part 标题节点" 规范成 part 下的一组 chapter：
    - [PartHome](part_url)
    - 其他 sidebar 分组/叶子
    """
    if not toc_tree:
    ...
    if len(toc_tree) == 1:
        ...
        root_url = (root.get("url") or "").strip() or None
        if _norm_title(root_title) == _norm_title(part_title):
            chapters = []
            if part_url:
                chapters.append({"title": part_title, "url": part_url, "children": []})
            if root_url and root_url != part_url:
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        for part in parts:
            ...
            toc_tree = _normalize_part_toc(str(part_title), str(part_url), toc_tree)
